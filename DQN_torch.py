    ...
    def train_network(self, dic_exp_conf):
        ...
        for i in range(epochs):        
            ...
            with torch.no_grad():
                ...
                # target_f = self.action_att_predict(self.states, self.critic_local)
                
            q_values = self.action_att_predict(self.states, self.critic_local).gather(1, self.actions.reshape(-1,1))

            # state_f=self.action_att_predict(self.states, self.critic_local)
            # for i, action in enumerate(self.actions):
            #     target_f[i][action] = target[i]
            # loss = torch.nn.MSELoss(reduction="none")(state_f, target_f).mean()
