...
class ColightNet(torch.nn.Module):
    ...
    def forward(self, state):
        agent = torch.tensor(state[0], dtype=torch.float32).to(device)
        adj = torch.tensor(state[1], dtype=torch.float32).to(device)
        ...
        # x = x[:,0,:]
        if self.name=='Actor':
            return torch.nn.Softmax(dim=2)(x), att_record
        else:
            return x, att_record
...
class SACAgentColight(Agent): 
    ...
    def action_att_predict(self,state, network,total_features=[],total_adjs=[]):
        ...
        all_output = network.forward([total_features,total_adjs])
        value, attention =all_output[0],all_output[1]
        if len(value)>1:
            return total_features,total_adjs,value,attention
        else:
    ...
    def choose_action(self, count, state):
        ...
        '''
        action_probabilities, attention=self.action_att_predict([state], self.actor_local)
        discrete_action = [np.random.choice(range(self.num_actions), p=probs) for probs in action_probabilities.cpu().detach().numpy().squeeze(0)]
        return discrete_action, attention
    ...
    def train_network(self, dic_exp_conf):
        ...
        with torch.no_grad():
            ...
            next_q_values_target = self.action_att_predict(self.next_states, self.critic_target)[2]   
            next_q_values_target2 = self.action_att_predict(self.next_states, self.critic_target2)[2]   

            soft_state_values = (action_probabilities * (torch.min(next_q_values_target, next_q_values_target2).to(device) - self.alpha * log_action_probabilities)).sum(dim=2)
            next_q_values = self.rewards / self.dic_agent_conf["NORMAL_FACTOR"] + self.dic_agent_conf["GAMMA"]*soft_state_values    
            
            if self.dic_traffic_env_conf["CONSTRAINT"]:
                next_q_values_target_cost = self.action_att_predict(self.next_states, self.critic_target_cost)[2]   
                next_q_values_target_cost2 = self.action_att_predict(self.next_states, self.critic_target_cost2)[2]   
                soft_state_values_cost = (action_probabilities * (torch.min(next_q_values_target_cost, next_q_values_target_cost2).to(device) - self.log_alpha.exp() * log_action_probabilities)).sum(dim=2)
                next_q_values_cost = self.cost + self.dic_agent_conf["GAMMA"]*soft_state_values_cost

        soft_q_values = self.action_att_predict(self.states, self.critic_local)[2].gather(2, self.actions.unsqueeze(2)).squeeze(2)
        soft_q_values2 = self.action_att_predict(self.states, self.critic_local2)[2].gather(2, self.actions.unsqueeze(2)).squeeze(2)
        ...
        if self.dic_traffic_env_conf["CONSTRAINT"]:
            soft_q_values_cost = self.action_att_predict(self.states, self.critic_local_cost)[2].gather(2, self.actions.unsqueeze(2)).squeeze(2)
            soft_q_values_cost2 = self.action_att_predict(self.states, self.critic_local_cost2)[2].gather(2, self.actions.unsqueeze(2)).squeeze(2)
    ...
    def actor_loss(self, states_tensor):
        ...
        q_values_local = self.action_att_predict(states_tensor, self.critic_local)[2]
        q_values_local2 = self.action_att_predict(states_tensor, self.critic_local2)[2]
        inside_term = (self.log_alpha.exp() * log_action_probabilities - torch.min(q_values_local, q_values_local2).to(device))
        
        if self.dic_traffic_env_conf["CONSTRAINT"]:
            q_values_cost = self.action_att_predict(states_tensor, self.critic_local_cost)[2]
            q_values_cost2 = self.action_att_predict(states_tensor, self.critic_local_cost2)[2]
            penalty = self.lam * torch.min(q_values_cost, q_values_cost2).to(device)
            policy_loss = (action_probabilities * (inside_term+penalty)).sum(dim=2).mean()
        else:
            policy_loss = (action_probabilities * inside_term).sum(dim=2).mean()
    ...
    def lambda_loss(self):
        q_cost = self.action_att_predict(self.states, self.critic_local_cost)[2].gather(2, self.actions.unsqueeze(2)).squeeze(2)
        q_cost2 = self.action_att_predict(self.states, self.critic_local_cost2)[2].gather(2, self.actions.unsqueeze(2)).squeeze(2)
    ...
    def get_action_info(self, states_tensor):
        ...
        _,_,action_probabilities,_=self.action_att_predict(states_tensor, self.actor_local)   
