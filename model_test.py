...
def test(model_dir, cnt_round, run_cnt, _dic_traffic_env_conf, if_gui):
    ...
    try:
        ...
        env = DIC_ENVS[dic_traffic_env_conf["SIMULATOR_TYPE"]](path_to_log=path_to_log,
                                                               ...
                                                               dic_traffic_env_conf=dic_traffic_env_conf, cnt_round=cnt_round)
        ...
        if dic_exp_conf["MODEL_NAME"] not in ['Fixedtime', 'MaxPressure']:
            for i in range(dic_traffic_env_conf['NUM_AGENTS']):
        ...
        while not done and step_num < int(dic_exp_conf["RUN_COUNTS"] / dic_traffic_env_conf["MIN_ACTION_TIME"]):
            ...
            for i in range(dic_traffic_env_conf["NUM_AGENTS"]):
                ...
                if dic_exp_conf["MODEL_NAME"] in ["CoLight", "SAC_Colight", "Colight_torch", "DQN_torch"]:
                    one_state = state
                ...
                elif "SAC_One" in dic_exp_conf["MODEL_NAME"]:
                    one_state = state
            ...
            next_state, reward, constraint, done, _ = env.step(action_list)
