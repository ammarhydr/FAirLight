...
class Updater:
    ...
    def __init__(self, cnt_round, dic_agent_conf, dic_exp_conf, dic_traffic_env_conf, dic_path, best_round=None, bar_round=None):
        ...
        env_tmp = DIC_ENVS[dic_traffic_env_conf["SIMULATOR_TYPE"]](
                              ...
                              dic_traffic_env_conf = self.dic_traffic_env_conf, cnt_round=cnt_round)        
        env_tmp.reset()
    ...
    def load_sample_for_agents(self):
        ...
        if self.dic_exp_conf['MODEL_NAME'] not in ["GCN","CoLight", "SAC_One", "SAC_Colight", "Colight_torch", "DQN_torch"]:
            ...
            if False: # Todo decide multi-process
                for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                    ...
                    if len(sample_set) == 0:
                        continue
                    samples_set_df = pd.DataFrame.from_records(sample_set,columns= ['state','action','next_state','inst_reward','reward','constraint','time','generator'])
                    samples_set_df['input'] = samples_set_df[['state','action','next_state','inst_reward','reward','constraint']].values.tolist()
                    samples_set_df.drop(['state','action','next_state','inst_reward','reward','constraint'], axis=1, inplace=True)
                ...
                for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                    ...
                    samples_set_df = pd.DataFrame.from_records(sample_set,columns= ['state','action','next_state','inst_reward','reward','constraint','time','generator'])
                    samples_set_df['input'] = samples_set_df[['state','action','next_state','inst_reward','reward','constraint']].values.tolist()
                    samples_set_df.drop(['state','action','next_state','inst_reward','reward','constraint','time','generator'], axis=1, inplace=True)
                    # samples_set_df['inter_id'] = i
    ...
    def update_network(self,i):
        ...
        if self.dic_traffic_env_conf["ONE_MODEL"]:
            ...
            elif self.dic_exp_conf['MODEL_NAME'] in ["SAC", "SAC_One", "SAC_Colight","Colight_torch", "DQN_torch"]:
