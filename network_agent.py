...
class NetworkAgent(Agent):
    ...
    def prepare_Xs_Y(self, memory, dic_exp_conf):
        ...
        for i in range(len(sample_slice)):
            state, action, next_state, reward, instant_reward, constraint, _, _ = sample_slice[i]
            # print(state)
            ...
            _next_state = []
            for feature_name in self.dic_traffic_env_conf["LIST_STATE_FEATURE"]:
                _state.append(np.array(state[feature_name]).reshape(-1,1).transpose())
                _next_state.append(np.array(next_state[feature_name]).reshape(-1,1).transpose())
